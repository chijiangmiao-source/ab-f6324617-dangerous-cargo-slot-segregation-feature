...
返回**首个冲突**的完整证据；无冲突则返回合规及实际比较对数。裁决之后还可为
待装箱推荐**离期望坐标最近的安全箱位**（曼哈顿壳层逐层搜索，同层取字典序首位）。
基于 Python 3.13 + FastAPI + Pydantic v2，规则与接口全量 pytest 覆盖，
...
│   ├── __init__.py
│   ├── main.py              # FastAPI 入口（/healthz、/api/v1/adjudicate、/api/v1/recommend-slot）
│   ├── models.py            # 请求/响应模型 + 整份输入校验（422）
│   └── rules.py             # 规则表、曼哈顿距离、裁决排序与壳层推荐搜索（纯函数）
├── tests/                   # 115 项 pytest 单元/接口测试
│   ├── test_rules.py
│   ├── test_api.py
│   ├── test_recommend.py
│   └── test_health.py
...
整舱裁决完成后，为待装箱寻找**离期望坐标最近的安全位置**。沿用舱段尺寸、
`Container` 模型与固定距离表；搜索只读，不改变已有箱位，只判断待装箱与
各现存箱的隔离关系。

#### 请求体

在裁决请求的基础上增加待装类别与期望坐标：

| 字段 | 类型 | 约束 |
|---|---|---|
| `max_row` / `max_col` / `max_tier` | int | 与裁决接口相同，**必须 ≥ 1** |
| `containers` | array | 现存箱位列表（可为空），约束同裁决接口 |
| `category` | string | 待装类别，**仅允许 `"A"` `"B"` `"C"` `"D"`** |
| `desired.row` / `desired.col` / `desired.tier` | int | 期望坐标，**从 1 开始**且不超出舱段容量 |

校验与裁决接口一致：类型错误、越界（**含期望坐标越界**）、现存箱重复占位、
未知类别、缺字段或未声明字段，**整份请求**返回 `422`，不返回任何推荐结果。
期望坐标恰好被现存箱占用**不算**非法：该位置在搜索中被排除，搜索自动向外扩展。

#### 搜索规则

* 从期望点按**曼哈顿距离逐层展开**：第 d 层为与期望点距离恰为 d 的全部
  舱内坐标，逐层增大 d，找到即停。
* 候选位置必须未被占用，且与**全部**现存箱的实际距离都达到要求距离
  （同一张固定规则表，临界相等为安全）。
* 只生成舱内壳层坐标（偏移量按舱段边界裁剪），**不为大尺寸稀疏舱段
  物化完整网格**。
* 同层存在多个候选（并列）时，取 `(排, 列, 层)` **字典序最小**者；
  结果与现存箱的输入顺序无关，唯一确定。

#### 响应

找到 → `200`，返回推荐坐标与搜索距离（推荐点与期望点的曼哈顿距离）：

```json
{ "status": "recommended", "slot": { "row": 4, "col": 5, "tier": 3 }, "distance": 1 }
```
...
curl -s http://localhost:8000/api/v1/recommend-slot \
  -H 'Content-Type: application/json' \
  -d '{"max_row":10,"max_col":10,"max_tier":5,
       "containers":[{"row":5,"col":5,"tier":3,"category":"D"}],
       "category":"A","desired":{"row":5,"col":5,"tier":3}}'
```
...
1. 等待 `api` 健康检查通过后启动；
2. 执行 170+ 项固定检查：三类特殊规则的**临界距离稳定判合规**、
   低一格组合产生**唯一冲突**、冲突证据由脚本侧**独立按曼哈顿距离复算**、
   多冲突首项排序及输入乱序不变性、比较对数、全部非法输入的整份 422，
   以及推荐接口的壳层搜索（空舱命中期望点、占用/不安全逐层外扩、
   候选须满足全部现存箱）、最近层并列取字典序首位且乱序不变、
   小舱无解返回无坐标的 `no_safe_slot`、推荐请求的整份 422；
...
覆盖（115 项）：规则矩阵完整性与对称性、曼哈顿三轴定义、非欧氏判定、
空位不折叠、临界相等合规、低一格冲突、首冲突双层排序、类别不参与排序、
20 次乱序可复现性、比较对数计数、三个健康地址与探针脚本退出码、全部
422 边界（尺寸非正、上下界越界、重复、未知类别、缺字段、多字段、类型错误、
空体），以及推荐接口的并列层字典序与乱序不变性、小舱无解无坐标状态、
占用/不安全候选排除、候选须满足全部现存箱、角点壳层裁剪、
10⁶×10⁶×10⁶ 稀疏大舱不物化完整网格、推荐请求整份 422。

## 设计说明

* `app/rules.py` 与 Web 层完全解耦，是无副作用的纯函数模块，可单独复用；
  规则表用无序类别对（`frozenset`）查表，对称适用且不存在未实现分支。
* 推荐搜索复用同一张规则表：按期望点的曼哈顿距离逐层生成**舱内壳层坐标**
  （偏移量先按边界裁剪），占用位与不安全位直接跳过，同层取字典序最小者；
  内存占用与舱段体积无关，稀疏大舱也不会物化完整网格。
* 校验在 Pydantic 模型层（含跨字段的上界与重复检测，聚合后一次拒绝），
  进入裁决/推荐逻辑的数据一定合法，故核心路径无需再处理非法输入。
